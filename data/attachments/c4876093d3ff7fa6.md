...
- Name: transferFunds\transferFunds.spec.js >> Transfer Funds Module >> TC08 - Transfer Confirmation Validation
- Location: tests\transferFunds\transferFunds.spec.js:77:10
...
```ts
  ...
  14  |           user = {
  15  |                ...registerData.user,
> 16  |                username: registerData.user.username + Date.now()
      |                                            ^ TypeError: Cannot read properties of undefined (reading 'username')
  17  |           };
  18  | 
  ...
  115 |           await transfer.verifyTransferSuccess();
  116 |           console.log('Transfer Workflow validation Done');
```